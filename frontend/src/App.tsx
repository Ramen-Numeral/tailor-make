...
function TraceList({ traces, running, verbose }: { traces: Trace[]; running: boolean; verbose: boolean }) {
  ...
  return <div className={`trace-list ${verbose ? "verbose" : "compact"}`}>{displayTraces.map((t, i) => <div className="trace-item" key={`${t.event_type}-${t.section ?? "workflow"}-${i}`}><div className="trace-rail"><span className={i === displayTraces.length - 1 && running ? "pulse" : ""}>{i + 1}</span></div>
    <div><div className="trace-meta">{t.section?.replaceAll("_", " ") || "workflow"} {t.attempt != null && `· attempt ${t.attempt}`}</div><h3>{t.title}</h3><p>{t.summary}</p>
      ...
      {t.job_requirements && t.job_requirements.length > 0 && verbose && <details className="trace-accordion"><summary>All {t.job_requirements.length} extracted requirements</summary><div className="requirement-list">{t.job_requirements.map((req, index) => {
        const tier = req.required ? "required" : req.importance === "supporting" ? "bonus" : "preferred";
        return <div className={`requirement-row ${tier}`} key={req.id}><span className="tier-symbol neutral">{String(index + 1).padStart(2, "0")}</span><div><b>{req.text}</b><small>{req.kind}</small></div><em>{tier}</em></div>;
      })}</div></details>}
      ...
      {t.coverage_plan && verbose && <details className="trace-accordion" open={Boolean(t.match_score)}><summary>Requirement support · {t.coverage_plan.requirement_matches.filter(match => match.support === "supported").length} supported · {t.coverage_plan.requirement_matches.filter(match => match.support === "partial").length} partial · {t.coverage_plan.requirement_matches.filter(match => match.support === "unsupported").length} unsupported</summary><div className="coverage-grid">{t.coverage_plan.requirement_matches.map(match => <div className={`coverage-box ${match.support}`} key={match.requirement_id}><div><span>{match.requirement_kind}</span><em>{match.importance}</em></div><strong>{match.requirement_text}</strong><small>{match.adjudication_reason || (match.support === "supported" ? "Direct source evidence found." : match.support === "partial" ? "Related evidence found, but the requirement is not fully demonstrated." : "No confirmed source evidence.")}</small><div className="support-label">{match.support === "supported" ? <Check /> : match.support === "partial" ? <span>≈</span> : <X />}{match.support}</div></div>)}</div></details>}
